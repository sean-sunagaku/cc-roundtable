...
export interface AgentProfileInputRecord {
  id?: string;
  name: string;
  description: string;
  enabledByDefault?: boolean;
...
export interface MeetingConfigRecord {
  id: string;
  topic: string;
  projectDir: string;
  members: string[];
...
const DEFAULT_AGENT_PROFILES: Array<Omit<AgentProfileRecord, "source">> = [
  {
    id: "product-manager",
...
export class LocalMeetingRoomSupport {
  private readonly summaryDirPath: string;
  private readonly agentDirPath: string;

  constructor(private readonly repoRoot: string) {
    this.summaryDirPath =
      process.env.MEETING_ROOM_SUMMARIES_DIR?.trim() ||
      path.resolve(this.repoRoot, ".claude/meeting-room/summaries");
    this.agentDirPath =
      process.env.MEETING_ROOM_AGENTS_DIR?.trim() ||
      path.resolve(this.repoRoot, ".claude/meeting-room/agents");
  ...
  saveMeetingSummary(payload: MeetingSummaryRecord): string {
  ...
  listAgentProfiles(): AgentProfileRecord[] {
    this.ensureDefaultAgentProfiles();
    const result = new Map<string, AgentProfileRecord>();
  ...
  saveAgentProfile(input: AgentProfileInputRecord): AgentProfileRecord {
    this.ensureDefaultAgentProfiles();
