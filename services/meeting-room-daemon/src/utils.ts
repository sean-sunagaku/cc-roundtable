...
export function resolveRepoRoot(): string {
  ...
  if (fs.existsSync(path.resolve(cwd, "electron")) && fs.existsSync(path.resolve(cwd, "docs"))) {
    return cwd;
  }
  if (fs.existsSync(path.resolve(cwd, "..", "electron"))) {
...
export function resolveWebFile(requestPath: string): string | null {
  ...
  const filePath = path.resolve(repoRoot, "apps/web/client", relative);
  if (!filePath.startsWith(path.resolve(repoRoot, "apps/web/client"))) {
    return null;
  }
  return filePath;
}
...
export function requireNodePty(): { spawn: (file: string, args: string[], options: Record<string, unknown>) => unknown } {
  return requireFromRepo(path.resolve(resolveRepoRoot(), "electron/node_modules/node-pty")) as {
