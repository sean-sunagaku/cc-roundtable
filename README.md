...
`npm --prefix electron run daemon:start*` はこのスクリプトのラッパーです。
...
```bash
MEETING_ROOM_DAEMON_PORT=5517 npm --prefix electron run daemon:start
MEETING_ROOM_DAEMON_TOKEN=secret npm --prefix electron run daemon:start:dev
```
