...
```
.claude/meeting-room/
  ├── docs/design/meeting-room-product-design.md
  ├── src/packages/meeting-room-hooks/
  │   ├── enforce-broadcast.py   # PreToolUse: directed メッセージをブロック
  │   └── ws-relay.py            # PostToolUse: WebSocket にメッセージ転送
  ├── src/apps/desktop/
  │   ├── package.json
  │   ├── tsconfig.json
  │   ├── src/
  ...
  │   │   │   ├── ws-server.ts   # WebSocket server (port 9999)
  │   │   │   ├── pty-manager.ts # node-pty で Claude Code を管理
  │   │   │   └── meeting.ts     # 会議セットアップ・ライフサイクル
  │   │   ├── renderer/
  │   │   │   ├── index.html
  │   │   │   ├── App.tsx
  │   │   │   ├── screens/
  │   │   │   │   ├── SetupScreen.tsx    # 会議セットアップ
  │   │   │   │   └── MeetingScreen.tsx  # 会議中
  │   │   │   ├── components/
  │   │   │   │   ├── ChatView.tsx       # メッセージリスト
  │   │   │   │   ├── MessageBubble.tsx  # 個別メッセージ
  │   │   │   │   ├── InputBar.tsx       # 入力欄
  │   │   │   │   ├── TerminalPane.tsx   # xterm.js ターミナル
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "python3 \"$HOME/.claude/meeting-room/src/packages/meeting-room-hooks/enforce-broadcast.py\""
          }
        ]
      }
    ],
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "python3 \"$HOME/.claude/meeting-room/src/packages/meeting-room-hooks/ws-relay.py\""
          }
        ]
      }
    ]
