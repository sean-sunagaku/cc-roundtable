...
`, "utf-8");
  ...
  submitPrompt(meetingId, prompt) {
    ...
    const delivered = this.ptyManager.write(meetingId, content);
    if (!delivered) return false;
    this.ptyManager.write(meetingId, "\n");
    return true;
  }
...
async function main() {
  const meetingId = `smoke_${Date.now()}`;
  ...
  pty2.on("data", (id, data) => {
    ...
    rawJoined += cleaned;
    if (rawJoined.length > 3e5) rawJoined = rawJoined.slice(-3e5);
    for (const line of cleaned.replace(/\r/g, "\n").split(/\n+/)) {
      const t = line.trim();
      if (!t) continue;
      ptyLines.push(t);
      if (ptyLines.length > 1200) ptyLines.shift();
    }
  });
  const config = {
    id: meetingId,
    skill: "feature-discussion",
    topic: "smoke-test-normal-chat",
    projectDir: "/Users/babashunsuke/Repository/cc-roundtable",
    members: []
  };
  console.log("[smoke] start meeting");
  service.startMeeting(config);
  const ready = await waitFor(() => /ClaudeCode|❯|bypass permissions/i.test(rawJoined), 3e4);
  console.log(`[smoke] prompt-ready: ${ready}`);
  const message = "2+2 \u306E\u7B54\u3048\u3060\u3051\u3092\u8FD4\u3057\u3066\u304F\u3060\u3055\u3044\u3002";
  const beforeRawLen = rawJoined.length;
  const delivered = service.sendHumanMessage(meetingId, message);
  console.log(`[smoke] sendHumanMessage: ${delivered}`);
  const gotAssistantMarker = await waitFor(() => /\n\s*⏺/.test(rawJoined.slice(beforeRawLen)), 18e4, 1500);
  const post = rawJoined.slice(beforeRawLen);
  const usageBlocked = /usage limit reached|weekly limit reached|limit has been reached/i.test(post);
  console.log(`[smoke] got-assistant-marker: ${gotAssistantMarker}`);
  console.log(`[smoke] usage-blocked-after-send: ${usageBlocked}`);
  console.log("[smoke] tail:");
  for (const line of ptyLines.slice(-70)) {
    console.log(line);
  }
