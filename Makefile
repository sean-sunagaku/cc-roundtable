.PHONY: help install dev daemon daemon-dev public-share public-share-dev public-share-ngrok public-share-ngrok-dev public-share-smoke public-share-api public-share-check lint format format-check typecheck build verify verify-web arch arch-new arch-update contracts contracts-check
...
lint: ## ESLint を実行
	npm --prefix src/apps/desktop run lint

format: ## Prettier で整形
	npm --prefix src/apps/desktop run format

format-check: ## Prettier の整形ズレを確認
	npm --prefix src/apps/desktop run format:check
...
verify: contracts-check lint format-check ## lint + format + typecheck + build + contracts-check + e2e:gui (リリース前に必ず実行)
