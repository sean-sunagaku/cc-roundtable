...
const api: MeetingRoomApi = {
  ...
  saveAgent: (input: AgentProfileInput): Promise<AgentProfile> =>
    ipc.invoke("meeting:save-agent", input),
  ...
  pickProjectDir: (currentDir?: string): Promise<string | null> =>
    ipc.invoke("meeting:pick-project-dir", currentDir),
  saveSummary: (payload: MeetingSummaryPayload): Promise<string> =>
    ipc.invoke("meeting:save-summary", payload),
  ...
  getSessionDebug: (meetingId: string): Promise<ClaudeSessionDebug> =>
    ipc.invoke("meeting:get-session-debug", meetingId),
  getSessionView: (meetingId: string): Promise<MeetingSessionView | null> =>
    ipc.invoke("meeting:get-session-view", meetingId),
  ...
  writeTerminal: (meetingId: string, data: string): Promise<boolean> =>
    ipc.invoke("meeting:terminal-write", meetingId, data),
  onTerminalData: (handler: (meetingId: string, chunk: string) => void): (() => void) =>
    ipc.on("terminal:data", handler),
  ...
  onTabsUpdate: (handler: (tabs: MeetingTab[]) => void): (() => void) =>
    ipc.on("meeting:tabs", handler),
  onRuntimeEvent: (handler: (event: RuntimeEvent) => void): (() => void) =>
    ipc.on("meeting:runtime-event", handler)
};
